export const icons = {
  bankBuilding: require("./icons/bank-building.svg").default,
  bankBuildingWhite: require("./icons/bank-building-white.svg").default,
  creditCard: require("./icons/credit-card.svg").default,
  payCash: require("./icons/pay-cash.svg").default,
  arrowLeft: require("./icons/arrow-left.svg").default,
  add: require("./icons/add.svg").default,
  phoneDial: require("./icons/phone-dial.svg").default,

};
